...
resource "azurerm_application_gateway" "main" {
  ...
  dynamic "request_routing_rule" {
    ...
    content {
      ...
      priority = (var.sku.tier == "Standard_v2" || var.sku.tier == "WAF_v2") ? request_routing_rule.value.priority : null
    }
  }
...
resource "azurerm_monitor_diagnostic_setting" "agw-diag" {
  ...
  dynamic "enabled_log" {
    iterator = log_category
    for_each = data.azurerm_monitor_diagnostic_categories.appgw.log_category_types
